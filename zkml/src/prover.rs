...
use crate::{
    claims::precommit, matrix::Matrix, model::{InferenceStep, InferenceTrace, Layer, Model, PolyID}, vector_to_mle, Claim, Element, VectorTranscript
};
    ...
    E: Serialize + DeserializeOwned,
{
    ...
    fn prove_step<'b>(
        ...
        match step.layer {
            ...
            Layer::Activation(crate::activation::Activation::Relu(relu)) => self.prove_relu(last_claim, input, &step.output)
        }
    }
    ...
    last_claim: Claim<E>,
    input: &[E],
    output: &[E]) -> anyhow::Result<Claim<E>> {
        Ok(last_claim)
    }
